...
class AutoAssignService:
    ...
    def run(self):
        with transaction.atomic():
            auto_items_qs = WorkItem.objects.filter(is_manual=False,
                session=self.session
            ).exclude(
                work_order__in=[KANBI_WO, DIRECT_WO]
            ).exclude(
                assignments__is_fixed=True
            ).distinct()
            ...
            Assignment.objects.filter(work_item_id__in=self.auto_item_ids, is_fixed=False).delete()
            ...
            # 3. 정렬된 기번 리스트
            sorted_gibuns = sorted(items_by_gibun.keys(), key=lambda g: gibun_orders.get(g, 999))
            ...
            for gibun in sorted_gibuns:
                ...
                team_items.sort(key=lambda x: (int(x.ordering or 0), -float(x.work_mh or 0.0)))
    ...
    def _assign_items_with_candidates(self, items, candidates, allow_over_limit):
        created = []
        if not candidates: return created, items
        ...
        for item in items:
            ...
            while remaining > 0.001:
                ...
                valid_cands = [w for w in candidates if self.temp_load[w.id] < w.limit_mh]
                if not valid_cands:
                    valid_cands = candidates

                min_load = min(self.temp_load[w.id] for w in valid_cands)
                targets = [w for w in valid_cands if abs(self.temp_load[w.id] - min_load) < 0.001]
            ...
            for w_id, amt in allocation.items():
                if amt > 0:
                    created.append(Assignment(work_item=item, worker_id=w_id, allocated_mh=round(amt,2), is_fixed=False))
...
class ScheduleSyncService:
    ...
    def run(self):
        with transaction.atomic():
            ...
            for item in shared_items:
                ...
                common_start = self._find_common_slot_forward(max_duration_min, involved_worker_ids, worker_timelines)
                if common_start is None: continue
    ...
    def _load_existing_timelines(self):
        ...
        fixed_assigns = Assignment.objects.filter(work_item__session=self.session, start_min__isnull=False, end_min__isnull=False)
        for a in fixed_assigns:
            s = a.start_min
            e = a.end_min
            if e < s: e += 1440
            timelines[a.worker.id].append((s, e))
        for w_id in timelines: timelines[w_id].sort()
        return timelines
...
def refresh_worker_totals(session):
    ...
    for w in workers:
        total = (w.assignments.filter(work_item__session=session)
