...
async function fixture(browser, name, templates = []) {
    ...
    await page.route('http://cb.test/**', async route => {
        ...
        let html = fs.readFileSync(path.join(root, 'templates/bookmarks', name === 'cb_template_manage' ? 'cb_open_list.html' : name + '.html'), 'utf8');
        html = html.replace(/{%\s*static\s+'([^']+)'\s*%}/g, '/static/$1')
            .replace(/{%\s*url\s+'bookmarks:cb_templates'\s*%}/g, '/api/templates')
            .replace(/{%\s*url\s+'bookmarks:cb_template_manage'\s*%}/g, '/manage')
        ...
        if (name === 'cb_template_manage') {
            const managed = templates[0] || { id: null, aircraft_model: 'B777', name: '', rows: [] };
...
test('managed template uses the document layout and saves edits', async () => {
    ...
    try {
        const record = {
            panel_loc: 'P11', cb_loc: 'A1', fin: '1QG1',
            description: 'TEST ITEM', cockpit: 'V', ee: '', etc: '', warning: '',
        };
        const result = await fixture(browser, 'cb_template_manage', [{
            id: 7, name: 'Engine', aircraft_model: 'B777', rows: [record],
        }]);
        const { page, errors } = result;
        assert.equal(await page.locator('.cb-open-sheet').count(), 1);
        assert.equal(await page.locator('#cbOpenManagedTemplateUpdate').count(), 1);
        assert.equal(await page.locator('#cbOpenListPrint').count(), 1);
        assert.equal(await page.locator('#cbManagedTemplateName').inputValue(), 'Engine');
        assert.equal(await page.locator('#cbOpenListBody [data-field="description"]').first().textContent(), 'TEST ITEM');
        await page.locator('#cbOpenListBody [data-field="description"]').first().fill('UPDATED ITEM');
        await page.click('#cbOpenManagedTemplateUpdate');
        await page.waitForFunction(() => document.querySelector('#cbOpenSaveMessage')?.textContent.includes('저장했습니다'));
...
test('open document can be saved as an aircraft template', async () => {
    ...
    try {
        ...
        await page.locator('#cbOpenListBody [data-field="panel_loc"]').first().fill('P11');
        await page.locator('#cbOpenListBody [data-field="description"]').first().fill('TEST ITEM');
        await page.click('#cbOpenTemplateSave');
        assert.equal(await page.locator('#cbOpenTemplateSaveDialog').isVisible(), true);
        await page.fill('#cbOpenTemplateName', 'Line check');
        await page.click('#cbOpenTemplateSaveConfirm');
        await page.waitForURL(/\/manage\?.*template_id=1/);
    ...
    try {
        for (const name of ['cb_open_list', 'cb_template_manage']) {
            ...
            await page.evaluate(name => {
                document.body.className = 'assignment-console-body cb-open-body';
            }, name);
